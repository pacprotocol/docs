...
``` bash
nano /etc/systemd/system/pacp.service # or any other file editor you want
```

With the following content of pacp.service:
...
Enable the service
```bash
systemctl enable pacp.service
```

And start the service

```bash
systemctl start pacp.service
```

If you want to stop the service (stopping the PAC Protocol Core Daemon instance)

```bash
systemctl stop pacp.service
```
